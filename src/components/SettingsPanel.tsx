  ...
  return (
    <>
      <button
        ...
        style={{
          color: (theme === 'nier' || theme === 'gnome') ? '#f5f0e8' : 'var(--text-primary)'
        }}
      ...
      {isOpen && (
        ...
        >
          <div className="p-3">
            <div>
              <label className={`block text-xs font-medium mb-2 ${theme === 'terminal' ? 'text-gray-400' :
                theme === 'neumorphic' ? 'text-gray-600' :
                  theme === 'nier' ? 'text-amber-900' :
                    theme === 'gnome' ? 'text-gray-300' :
                      'text-gray-700'
                }`}>
                Theme
              </label>
              <div className="space-y-1">
                {themes.map((themeOption) => {
                  const getThemeButtonClasses = (buttonTheme: ThemeType) => {
                    ...
                    switch (buttonTheme) {
                      case 'terminal':
                        return `${baseClasses} ${fontClass} rounded-none ${isSelected
                          ? 'border-white bg-black text-white'
                          : 'border-gray-600 bg-gray-900 text-gray-300 hover:border-gray-500'
                          }`;
                      case 'modern':
                        return `${baseClasses} ${fontClass} rounded ${isSelected
                          ? 'border-blue-300 bg-white text-blue-600 shadow-sm'
                          : 'border-gray-200 bg-gray-50 text-gray-700 hover:bg-white hover:shadow-sm'
                          }`;
                      case 'neumorphic':
                        return `${baseClasses} ${fontClass} transition-all rounded-lg border-transparent ${isSelected
                          ? 'text-purple-700 bg-gray-200'
                          : 'text-gray-600 bg-gray-200'
                          }`;
                      case 'nier':
                        return `${baseClasses} ${fontClass} border-amber-900 rounded-none font-semibold ${isSelected
                          ? 'bg-amber-100 text-amber-900 border-2'
                          : 'bg-amber-50 text-amber-800 hover:bg-amber-100 border-2'
                          }`;
                      case 'gnome':
                        return `${baseClasses} ${fontClass} rounded-none ${isSelected
                  ...
                  const getThemeButtonStyle = (buttonTheme: ThemeType) => {
                    ...
                    switch (buttonTheme) {
                      case 'neumorphic':
                        ...
                        break;
                      case 'gnome':
                        baseStyle.backgroundColor = isSelected ? '#06989a' : '#300a24';
                        baseStyle.borderColor = isSelected ? '#06989a' : '#4a4a4a';
                        break;
                    }
