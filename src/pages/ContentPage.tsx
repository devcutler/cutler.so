...
export function ContentPage() {
	...
	const getTextStyles = () => {
		return {
			...
			tag: { color: 'var(--accent-color)', backgroundColor: 'var(--bg-tertiary)' }
		};
	};
		...
		return (
			<div className={`${getContainerClasses()} ${theme === 'terminal' ? 'py-0' : 'text-center py-12'}`}>
				<div className={getContentClasses()}>
					{theme === 'terminal' ? (
						<>
							<div className="font-mono text-xl mb-4" style={textStyles.error}>404 - File not found</div>
							<div className="font-mono mb-4" style={textStyles.muted}>The requested page does not exist.</div>
							<a href="/" className="font-mono text-sm underline hover:opacity-80 transition-colors" style={textStyles.link}>Return to home</a>
						</>
					) : (
						<>
							<h1 className={`text-4xl font-bold mb-4 ${textClasses.error}`} style={textStyles.error}>404</h1>
							<p className={`${textClasses.muted} mb-8`} style={textStyles.muted}>Page not found</p>
							<a href="/" className={textClasses.link} style={textStyles.link}>Go home</a>
						</>
					)}
				</div>
			</div>
		...
		<article className={getContainerClasses()}>
			<div className={getContentClasses()}>
				{theme === 'terminal' ? (
					<>
						{content.date && (
							<div className={`${textClasses.date} mb-2`} style={textStyles.date}>
                # Created: {formatDate(content.date)}
						...
						{content.tags && content.tags.length > 0 && (
							<div className="mb-4">
								<div className="font-mono text-sm" style={textStyles.muted}># Tags:</div>
								<div className="flex flex-wrap">
									{content.tags.map(tag => (
										<span key={tag} className={textClasses.tag} style={textStyles.tag}>#{tag}</span>
									))}
								</div>
							</div>
						)}
					...
					<>
						{content.date && (
							<div className={`text-sm ${textClasses.date} mb-4`} style={textStyles.date}>
						...
						{content.tags && content.tags.length > 0 && (
							<div className="mt-8 pt-8 border-t" style={{ borderColor: 'var(--border-color)' }}>
								<div className="flex flex-wrap gap-2">
									{content.tags.map(tag => (
										<span key={tag} className={textClasses.tag} style={textStyles.tag}>
											{tag}
										</span>
									))}
								</div>
